...
void pretty_print_sam_unaligned(pretty * pa) {
...
pretty * pretty_from_string_fast(char* sam_string) {
	...
	if (*current=='\0') {
		fprintf(stderr,"NOT IN SAM FORMAT!\n");
	}
	*current='\0';
	pa->read_name=strdup(sam_string);
	pa->read_name_length=current-sam_string;
	*current='\t';
	//Get the flag
	current++;
	...
	*current='\0';
	pa->flags=atoi(previous);
	*current='\t';
	current++;
	while ( *current!='\t' && *current!='\0') { current ++; }; //rname
	if (*current=='\0') {
		fprintf(stderr,"NOT IN SAM FORMAT!\n");
	}
	current++;
	while ( *current!='\t' && *current!='\0') { current ++; }; //pos
	if (*current=='\0') {
		fprintf(stderr,"NOT IN SAM FORMAT!\n");
	}
	current++;
	while ( *current!='\t' && *current!='\0') { current ++; }; //mapq
	if (*current=='\0') {
		fprintf(stderr,"NOT IN SAM FORMAT!\n");
	}
	current++;
	while ( *current!='\t' && *current!='\0') { current ++; }; //cigar
	if (*current=='\0') {
		fprintf(stderr,"NOT IN SAM FORMAT!\n");
	}
	current++;
	while ( *current!='\t' && *current!='\0') { current ++; }; //mrnm
	if (*current=='\0') {
		fprintf(stderr,"NOT IN SAM FORMAT!\n");
	}
	current++;
	while ( *current!='\t' && *current!='\0') { current ++; }; //mpos
	if (*current=='\0') {
		fprintf(stderr,"NOT IN SAM FORMAT!\n");
	}
	current++; previous=current;
	while ( *current!='\t' && *current!='\0') { current ++; }; //isize
	if (*current=='\0') {
		fprintf(stderr,"NOT IN SAM FORMAT!\n");
	}
	*current='\0';
	pa->isize=atoi(previous);
	*current='\t';
	current++;
	while ( *current!='\t' && *current!='\0') { current ++; }; //seq
	if (*current=='\0') {
		fprintf(stderr,"NOT IN SAM FORMAT!\n");
	}
	current++;
	while ( *current!='\t' && *current!='\0') { current ++; }; //qual
	if (*current=='\0') {
		pa->has_score=false;
	} else {
		current++;
		if (current[0]=='A' && current[1]=='S' && current[2]==':' && current[3]=='i' && current[4]==':') {
			current+=5; previous=current;
			while ( *current!='\t' && *current!='\0') { current ++; }; //qual
			if (*current=='\0') {
				pa->has_score=false;
			} else {
